...
def mode_render():
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .hk-grid { display: grid; grid-template-columns: 1.1fr 0.9fr; gap: 60px; z-index: 1; flex-grow: 1; min-height: 0; margin-bottom: 20px; }
    .hk-main-title { font-family: var(--serif); font-size: clamp(1.8rem, 3.2vw, 3.8rem); line-height: 1.2; font-weight: 900; letter-spacing: -1px; margin-bottom: 15px; }
    .hk-eng-subtitle { font-size: 0.8rem; color: #888; text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 30px; line-height: 1.4; }
    ...
    .hk-core-statement p { font-size: 1.2rem; font-family: var(--serif); line-height: 1.5; font-weight: 700; color: var(--hk-black); }
    ...
    h3 { font-family: var(--serif); font-size: 1.4rem; margin: 30px 0 15px; border-bottom: 2px solid var(--hk-black); display: inline-block; }
    p { font-size: 1.05rem; line-height: 1.8; margin-bottom: 18px; text-align: justify; }
    
    strong, b { color: var(--hk-red); font-weight: 700; } 
    li { font-size: 1.05rem; padding: 10px 0; border-bottom: 1px solid var(--hk-gray); display: flex; gap: 10px; line-height: 1.6; }
    li::before { content: '→'; font-weight: 900; color: var(--hk-red); flex-shrink: 0; }
    
    .hk-footer { flex-shrink: 0; display: flex; justify-content: space-between; align-items: flex-end; border-top: 1px solid #000; padding-top: 15px; z-index: 2; background: var(--hk-bg); }
    .hk-logo { font-weight: 900; letter-spacing: 2px; font-size: 1rem; }
    .hk-logo span { color: var(--hk-red); }
    .hk-scroll-hint { font-size: 0.7rem; letter-spacing: 2px; font-weight: 700; }
    """
    ...
    with open(OUTPUT_HTML, "w", encoding="utf-8") as f:
        f.write(full_html)
    print(f"[Render] {OUTPUT_HTML} 更新完成（修正背景文字重疊）。")
    git_push_auto()
