...
def git_push_auto():
    try:
        ...
        commit_msg = f"🤖 Poster Uniform Font Edition: {datetime.now().strftime('%m-%d %H:%M')}"
        subprocess.run(["git", "commit", "-m", commit_msg], capture_output=True)
        subprocess.run(["git", "push", "origin", "main"], check=True)
        print("✅ [Git Push Completed] 網頁已更新！")
...
def mode_render():
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    /* 核心句與正文字體統一 */
    .core-statement { 
        font-size: 1.3rem; /* 統一字體大小 */
        background: #f8f8f8; padding: 25px; border-left: 8px solid #000; margin-bottom: 40px; line-height: 1.6; 
    }
    ...
    main p, main li { 
        font-size: 1.3rem; /* 統一字體大小 */
        margin-bottom: 20px; line-height: 1.8; text-align: justify; 
    }
    main ul, main ol { margin-bottom: 30px; padding-left: 25px; }
